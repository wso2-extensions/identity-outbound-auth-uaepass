...
public class UAEPassAuthenticator extends AbstractApplicationAuthenticator
        ...
        try {
            ...
            if (authenticatorProperties != null) {
                ...
                OAuthClientRequest authzRequest = OAuthClientRequest
                        ...
                        .setResponseType(UAEPassAuthenticatorConstants.UAE.OAUTH2_GRANT_TYPE_CODE)
                        .setState(state)
                        .buildQueryMessage();
                ...
                if (StringUtils.isNotBlank(authenticatorProperties.get(UAEPassAuthenticatorConstants.UAE.QUERY_PARAMS))) {
    ...
    protected void processAuthenticationResponse(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            jsonClaimMap.entrySet().stream()
                    .filter(entry -> !ArrayUtils.contains(UAEPassAuthenticatorConstants.UAEPassRuntimeConstants.
    ...
    public String processAdditionalQueryParamSeperation(Map<String, String> authenticatorProperties, String loginPage)
        ...
        if(additionalQueryParams.contains(",")){
            retval = additionalQueryParams.split(",");
        } else {
            retval = additionalQueryParams.split("&");
        }
    ...
    /**
     * This method is used to retrieve user claims from user info endpoint.
     *
     * @param oAuthResponse
     * @param context
     * @return
     ...
     * @throws IOException
     */
    public String sendUserInfoRequest(AuthenticationContext context, String accessToken) throws
            UAEPassUserInfoFailedException {
     ...
     */
    public String getAuthenticatedUserId(AuthenticationContext context, Map<String, Object> idTokenClaims) throws AuthenticationFailedException {
        ...
        if (isUserIdFoundAmongClaims(context)) {
            ...
            if (StringUtils.isNotBlank(authenticatedUserId)) {
                ...
                authenticatedUserId = getAuthenticatedUser(idTokenClaims);
            }
        } else {
            authenticatedUserId = getAuthenticatedUser(idTokenClaims);
            if (LOG.isDebugEnabled()) {
        ...
        if (StringUtils.isBlank(authenticatedUserId)) {
            throw new AuthenticationFailedException(
                    UAEPassAuthenticatorConstants.ErrorMessages.USER_ID_NOT_FOUND_IN_ID_TOKEN_SENT_BY_FEDERATED_IDP.getCode(),
                    UAEPassAuthenticatorConstants.ErrorMessages.USER_ID_NOT_FOUND_IN_ID_TOKEN_SENT_BY_FEDERATED_IDP.getMessage());
        }
    ...
    /**
     * Checks whether the user id is available in the claims
     *
     * @param context
     * @return
     */
     ...
     * @throws AuthenticationFailedException
     */
    public String getSubjectFromUserIDClaimURI(AuthenticationContext context, Map<String, Object> idTokenClaims)
            throws AuthenticationFailedException {
        ...
        try {
            ...
            throw new AuthenticationFailedException(
                    UAEPassAuthenticatorConstants.ErrorMessages.EXECUTING_CLAIM_TRANSFORMATION_FOR_IDP_FAILED.getCode(),
                    String.format(UAEPassAuthenticatorConstants.ErrorMessages.EXECUTING_CLAIM_TRANSFORMATION_FOR_IDP_FAILED.
                                    getMessage(), context.getExternalIdP().getIdPName()), ex);
        }
        if (LOG.isDebugEnabled()) {
    ...
    public String getUserIdClaimUriInOIDCDialect(String userIdClaimInLocalDialect, String spTenantDomain)
        ...
        List<ExternalClaim> externalClaims = UAEPassDataHolder.getInstance()
                .getClaimMetadataManagementService().getExternalClaims(UAEPassAuthenticatorConstants.
                                UAEPassRuntimeConstants.OIDC_DIALECT, spTenantDomain);
    ...
    public String getMultiAttributeSeparator(AuthenticationContext context, String authenticatedUserId)
        ...
        try {
            ...
            UserRealm userRealm = UAEPassDataHolder.getInstance().getRealmService()
            ...
            throw new AuthenticationFailedException(UAEPassAuthenticatorConstants.ErrorMessages.
                    RETRIEVING_MULTI_ATTRIBUTE_SEPARATOR_FAILED.getCode(),
                    UAEPassAuthenticatorConstants.ErrorMessages.RETRIEVING_MULTI_ATTRIBUTE_SEPARATOR_FAILED.getMessage(),
    ...
    public OAuthClientRequest getAccessTokenRequest(AuthenticationContext context, OAuthAuthzResponse authzResponse)
        ...
        try {
            ...
            throw new UAEPassAuthnFailedException("Error occurred while extracting the absolute public URL from browser.", e);
        }
    ...
    /**
     * Returns the Authorize URL of the UAEPass based on selected environment. First this method check the if there is
     * valid key in the XML file configs. Other vice it will pick the default URL.
     *
     * @param envUAEPass
     * @return
     */
    public String getAuthorizeUrl(String envUAEPass) {
        ...
        if (StringUtils.equals(envUAEPass, UAEPassAuthenticatorConstants.UAEPassRuntimeConstants.STAGING)) {
            if (StringUtils.isBlank(getAuthenticatorConfig().getParameterMap().get(
                    UAEPassAuthenticatorConstants.Endpoints.StagingEndpointKeys.UAEPASS_STG_AUTHZ_ENDPOINT_KEY))) {
                return UAEPassAuthenticatorConstants.Endpoints.StagingEndpointValues.UAEPASS_STG_AUTHZ_ENDPOINT_VALUE;
            } else {
                return getAuthenticatorConfig().getParameterMap().get
                        (UAEPassAuthenticatorConstants.Endpoints.StagingEndpointKeys.UAEPASS_STG_AUTHZ_ENDPOINT_KEY);
            }
        } else {
            if (StringUtils.isBlank(getAuthenticatorConfig().getParameterMap().get(
                    UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointKeys.UAEPASS_PROD_AUTHZ_ENDPOINT_KEY))) {
                return UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointValues.UAEPASS_PROD_AUTHZ_ENDPOINT_VALUE;
            } else {
                return getAuthenticatorConfig().getParameterMap().get
                        (UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointKeys.UAEPASS_PROD_AUTHZ_ENDPOINT_KEY);
            }
    ...
    /**
     * Returns the Token URL of the UAEPass based on selected environment. First this method check the if there is
     * valid key in the XML file configs. Other vice it will pick the default URL.
     *
     * @param envUAEPass
     * @return
    ...
    public String getTokenUrl(String envUAEPass) {
        ...
        if (StringUtils.equals(envUAEPass, UAEPassAuthenticatorConstants.UAEPassRuntimeConstants.STAGING)) {
            if (StringUtils.isBlank(getAuthenticatorConfig().getParameterMap().get(
                    UAEPassAuthenticatorConstants.Endpoints.StagingEndpointKeys.UAEPASS_STG_TOKEN_ENDPOINT_KEY))) {
            ...
            if (StringUtils.isBlank(getAuthenticatorConfig().getParameterMap().get(
                    UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointKeys.UAEPASS_PROD_TOKEN_ENDPOINT_KEY))) {
                return UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointValues.UAEPASS_PROD_TOKEN_ENDPOINT_VALUE;
            } else {
                return getAuthenticatorConfig().getParameterMap().get
                        (UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointKeys.UAEPASS_PROD_TOKEN_ENDPOINT_KEY);
            }
    ...
    /**
     * Returns the UserInfo URL of the UAEPass based on selected environment. First this method check the if there is
     * valid key in the XML file configs. Other vice it will pick the default URL.
     *
     * @param envUAEPass
     * @return
    ...
    public String getUserInfoUrl(String envUAEPass) {
        ...
        if (StringUtils.equals(envUAEPass, UAEPassAuthenticatorConstants.UAEPassRuntimeConstants.STAGING)) {
            if (StringUtils.isBlank(getAuthenticatorConfig().getParameterMap().get(
                    UAEPassAuthenticatorConstants.Endpoints.StagingEndpointKeys.UAEPASS_STG_USER_INFO_ENDPOINT_KEY))) {
                return UAEPassAuthenticatorConstants.Endpoints.StagingEndpointValues.UAEPASS_STG_USER_INFO_ENDPOINT_VALUE;
            } else {
                return getAuthenticatorConfig().getParameterMap().get
                        (UAEPassAuthenticatorConstants.Endpoints.StagingEndpointKeys.UAEPASS_STG_USER_INFO_ENDPOINT_KEY);
            }
            ...
            if (StringUtils.isBlank(getAuthenticatorConfig().getParameterMap().get(
                    UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointKeys.UAEPASS_PROD_USER_INFO_ENDPOINT_KEY))) {
                return UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointValues.UAEPASS_PROD_USER_INFO_ENDPOINT_VALUE;
            } else {
                return getAuthenticatorConfig().getParameterMap().get
                        (UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointKeys.UAEPASS_PROD_USER_INFO_ENDPOINT_KEY);
            }
    ...
    /**
     * Returns the Logout URL of the UAEPass based on selected environment. First this method check the if there is
     * valid key in the XML file configs. Other vice it will pick the default URL.
     *
     * @param envUAEPass
     * @return
    ...
    public String getLogoutUrl(String envUAEPass) {
        ...
        if (StringUtils.equals(envUAEPass, UAEPassAuthenticatorConstants.UAEPassRuntimeConstants.STAGING)) {
            if (StringUtils.isBlank(getAuthenticatorConfig().getParameterMap().get(
                    UAEPassAuthenticatorConstants.Endpoints.StagingEndpointKeys.UAEPASS_STG_LOGOUT_ENDPOINT_KEY))) {
            ...
            if (StringUtils.isBlank(getAuthenticatorConfig().getParameterMap().get(
                    UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointKeys.UAEPASS_PROD_LOGOUT_ENDPOINT_KEY))) {
                return UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointValues.UAEPASS_PROD_LOGOUT_ENDPOINT_VALUE;
            } else {
                return getAuthenticatorConfig().getParameterMap().get
                        (UAEPassAuthenticatorConstants.Endpoints.ProductionEndpointKeys.UAEPASS_PROD_LOGOUT_ENDPOINT_KEY);
            }
    ...
    public boolean isStagingEnvSelected(AuthenticationContext context){
        Map<String, String> authenticatorProperties = context.getAuthenticatorProperties();
        boolean isStagingEnv = Boolean.parseBoolean(authenticatorProperties.get(UAEPassAuthenticatorConstants.
                UAE.UAEPASS_ENV));
        if(isStagingEnv){
            return true;
        } else {
    ...
    public boolean isLogoutEnabled(AuthenticationContext context) {
        Map<String, String> authenticatorProperties = context.getAuthenticatorProperties();
        boolean isLogoutEnabled = Boolean.parseBoolean(authenticatorProperties
                .get(UAEPassAuthenticatorConstants.UAE.LOGOUT_ENABLE));
        if(isLogoutEnabled){
            return true;
        } else {
            return false;
